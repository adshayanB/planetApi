...
db=SQLAlchemy(app)
ma = Marshmallow(app) # Creates Seralilzer
jwt=JWTManager(app)
mail=Mail(app)
...
def planets():
    planetsList=Planet.query.all() #Returns a list of objects this is not json serializable (using jsonify )
...
def login ():
    ...
    if test:
        ...
        return jsonify(message='Login succeeded!', accessToken=accessToken)
    else:
        return jsonify(message='Entered bad email or password'),401
...
@app.route('/registerPlanet',methods=['POST'])
@jwt_required
def registerPlanet():
    ...
    if test:
        return jsonify(message='Planet with that name exists'),409
    else:
        ...
        newPlanet=Planet(planet_name=planet_name,
...
class PlanetSchema(ma.Schema):
    class Meta:
        fields=('planet_id','planet_name','planet_type','home_star','radius','distance')

#deseralize one object
user_Schema=UserSchema()
planet_Schema=PlanetSchema()
#deseralize mutliple objects
